...
import {
  determineCutLineValue,
  getGolferStatsFromLeaderboardData,
} from './golferStats';
...
httpGetAsync(mastersLeaderboard, async (stats: any) => {
  // async function main() {
  ...
  const playersStatsWithMoney = await calculatePrizeMoneyForEachPlayer(
    playersStats,
    prizeMoneyList,
    cutline
  );
  const entrantsPlayerData = await populateGolferData(
    entrants,
    playersStatsWithMoney
  );

  // Sort the leaderboard by prize money
  entrantsPlayerData.sort((a, b) =>
    a.prizeMoneyTotal > b.prizeMoneyTotal ? -1 : 1
  );
