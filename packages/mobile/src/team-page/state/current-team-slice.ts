...
const currentTeam = createSlice({
  ...
  reducers: {
    ...
    removeGolferId(state, action: PayloadAction<number>) {
      state.team.golferIds = state.team.golferIds.filter((golferId) => golferId !== action.payload);
    },
...
export const { setTeam, addGolferId, removeGolferId, setGolferIds, setGolfersRef, setIsNewTeam } = currentTeam.actions;
