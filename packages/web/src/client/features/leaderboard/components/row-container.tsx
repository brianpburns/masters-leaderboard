...
export const RowContainer = ({ position, row }: Props) => {
  ...
  const displaySubTable = !!golfers && Object.keys(golfers).length > 0 && !selectionPhase;
  ...
  return (
    <>
      <PrimaryRow open={open} setOpen={toggleDropdown} position={position} row={row} selectionPhase={selectionPhase} />
