...
export const PrimaryRow = ({ open, setOpen, position, row, selectionPhase }: Props) => (
  <TableRow onClick={() => setOpen(!open)} hover={true} selected={open}>
    <StyledTableCell>
      {!selectionPhase && (
        <IconButton aria-label="expand row" size="small">
          {open ? <KeyboardArrowUp data-testid="toggle-up" /> : <KeyboardArrowDown data-testid="toggle-down" />}
        </IconButton>
      )}
    </StyledTableCell>
